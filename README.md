...
当前功能：全球城市搜索、当前位置天气、24 小时与 16 天预报、空气质量、生活指数、动态穿衣指南、未来 2 小时风险卡片、过去 7 天与未来 16 天趋势曲线、可选日期历史天气查询、卫星云图、高清城市底图、雷达回放、温度/湿度/风/气压/降水图层、CMA 与 NHC 台风路径、台风风圈与预报锥、预警列表。地图默认定位所选城市，云图透明度、风场尺度与阵风提示会随交互变化；放大城市时天气网格自动加密。天气支持手动刷新并约每 15 分钟检查，台风约每 10 分钟检查，地图雷达约每 5 分钟检查。
...
当前真实色图层是 NASA GIBS 的 VIIRS/MODIS 产品，云图透明度可以降低整张卫星影像的遮挡感，并通过城市标记保持所选城市清晰置顶；“高清城市”切换到 Esri World Imagery，可放大查看城市细节，但它是底图/影像，不是实时云图。VIIRS 适合区域级观测，不是米级实时城市底图；Himawari 红外更新更频繁但空间分辨率更低。下一阶段若接入后端，可用 Sentinel-2 10 米多光谱 + 云概率做多时相无云合成，阴雨时再叠加 Sentinel-1 SAR（全天候、雷达质感），但这需要合适的数据服务、配额和缓存策略。
